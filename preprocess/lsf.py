...
cmd = 'singularity exec groseq_latest.sif groseq --threads {{threads}} -f {{fasta}} -a mm10.refGene.bed -g ./mm10 -o {{output}} --chromInfo mm10.chrom.sizes'.format(pwd=os.getcwd(), fasta=fasta, output=output, threads=len(job_hosts))
