...
def download_file(url, dest=None, overwrite=False):
	if not overwrite and dest and os.path.isfile(dest):
		print('Downloading file "{}" ==> "{}". Already exists, skipping...'.format(url, dest))
		return dest
	...
	if re.match("^(http|ftp)", url):
		...
		with tempfile.NamedTemporaryFile(mode="wb", delete=False) as file:
			path = file.name
			progress_bar.set_description('Downloading file "{}" ==> "{}"...'.format(url, dest if dest else path))
...
def download_raw_genome(url, dest, overwrite=False):
	if not overwrite and dest and os.path.isfile(dest):
		print('Downloading raw genome "{}". Already exists, skipping...'.format(dest))
		return

	print('Downloading raw genome "{}"\n'.format(url))
...
def download_bowtie2_index(url, dest, overwrite=False):
	if not overwrite and dest and os.path.isfile(dest):
		print('Downloading bowtie2 index "{}". Already exists, skipping...'.format(dest))
		return
