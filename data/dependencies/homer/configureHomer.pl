...
if ($getFactsFlag == 1) {
    ...
    print STDOUT "wget -O homer.package.zip \"$url\" [1]";
    #`sleep 5`;
...
#	exit 1;
	...
	if ($installFlag) {
	    print STDOUT "configureHomer.pl is not updated";
        #`sleep 5`;
		...
		foreach(@groups) {
			...
			foreach(keys %{$installList{$mode}}) {
				...
				print STDERR "$url";
				`wget -O homer.package.zip "$url"`;
				print STDOUT "wget -O homer.package.zip \"$url\" [2]";
		        #`sleep 5`;
...
sub updateConfigureScript {
	...
	my $tmpFile = rand() . ".tmp";
	print STDERR "`wget -O $tmpFile $scriptURL`; [3]\n";
